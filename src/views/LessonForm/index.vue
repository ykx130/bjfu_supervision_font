          ...
          for (let lesson in this.lessons) {
            for (let lesson_case in this.lessons[lesson].lesson_cases) {
              ...
              if (time.indexOf("0102") > -1) {
                if (flag[0][weekday] === 0) {
                  this.data[0][weekday].push(this.lessons[lesson]);
                  console.log(this.data[0][weekday].length);
                  this.data[0][weekday][this.data[0][weekday].length - 1].week = this.getWeek(lesson, weekday, '0102').slice(0);
                  console.log(this.getWeek(lesson, weekday, '0102'));
                  console.log(this.data[0][3]);//
                  flag[0][weekday] = 1;
                }
              }
              if (time.indexOf("0304") > -1) {
                if (flag[1][weekday] === 0) {
                  this.data[1][weekday].push(this.lessons[lesson]);
                  this.data[1][weekday][this.data[1][weekday].length - 1].week = this.getWeek(lesson, weekday, '0304').slice(0);
                  flag[1][weekday] = 1;
                }
              }
              if (time.indexOf("05") > -1) {
                if (flag[2][weekday] === 0) {
                  this.data[2][weekday].push(this.lessons[lesson]);
                  this.data[2][weekday][this.data[2][weekday].length - 1].week = this.getWeek(lesson, weekday, '05').slice(0);
                  flag[2][weekday] = 1;
                  console.log(this.getWeek(lesson, weekday, '05'));
                  console.log(this.data[2][3]);
                  console.log(this.data[0][3]);//
                }
              }
              if (time.indexOf("0607") > -1) {
                if (flag[3][weekday] === 0) {
                  this.data[3][weekday].push(this.lessons[lesson]);
                  this.data[3][weekday][this.data[3][weekday].length - 1].week = this.getWeek(lesson, weekday, '0607').concat();
                  flag[3][weekday] = 1;
                }
              }
              if (time.indexOf("0809") > -1) {
                if (flag[4][weekday] === 0) {
                  this.data[4][weekday].push(this.lessons[lesson]);
                  this.data[4][weekday][this.data[4][weekday].length - 1].week = this.getWeek(lesson, weekday, '0809').concat();
                  flag[4][weekday] = 1;
                }
              }
              if (time.indexOf("1011") > -1) {
                if (flag[5][weekday] === 0) {
                  this.data[5][weekday].push(this.lessons[lesson]);
                  this.data[5][weekday][this.data[5][weekday].length - 1].week = this.getWeek(lesson, weekday, '1011').concat();
                  flag[5][weekday] = 1;
                }
              }
              if (time.indexOf("12") > -1) {
                if (flag[6][weekday] === 0) {
                  this.data[6][weekday].push(this.lessons[lesson]);
                  this.data[6][weekday][this.data[6][weekday].length - 1].week = this.getWeek(lesson, weekday, '12').concat();
                  flag[6][weekday] = 1;
                }
              }
            }
