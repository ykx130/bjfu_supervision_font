<template>
  <div :data="form_meta">
    ...
    <div>
      ...
      <br/>-->
      <Input placeholder="授课教师" v-model="form.meta.teacher" v-bind:style="{ fontSize:'15px',width:'400px' }"></Input>
      <Input placeholder="上课班级" v-model="form.meta.class" v-bind:style="{ fontSize:'15px',width:'400px' }"></Input>
      <Input placeholder="上课地点" v-model="form.meta.place" v-bind:style="{ fontSize:'15px',width:'400px' }"></Input>
    </div>
    <br/>
    <div v-for="it in form_meta.items">
      ...
      <div v-if="it.item_type === 'radio_options'">
        ...
        <RadioGroup>
          <Radio v-model="form.values.value" v-for="op in it.payload.options" :label="op.label" :key="op.value" v-bind:style="{ fontSize:'15px',marginLeft:'25px' }">
            <span>{{op.label}}</span>
          </Radio>
        </RadioGroup>
      </div>
      <div v-if="it.item_type === 'checkbox_options'">
        <h1 style="height: 70px;line-height: 70px;margin-left: 20px">{{ it.extra }}</h1>
        <CheckboxGroup>
          <Checkbox v-model="form.values.value" v-for="op in it.payload.options" :label="op.label" :key="op.value" v-bind:style="{ fontSize:'15px',marginLeft:'25px' }">
            <span>{{op.label}}</span>
          </Checkbox>
        </CheckboxGroup>
      </div>
      <div v-if="it.item_type === 'raw_text'">
        <h1 style="height: 70px;line-height: 70px;margin-left: 20px">{{ it.extra }}</h1>
        <Input v-model="form.values.value" type="textarea" placeholder="Satisfation about teachers..." v-bind:style="{marginLeft:'25px',width:'85%'}"></Input>
      </div>
