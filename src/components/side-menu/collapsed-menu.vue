  ...
  <Dropdown @on-click="handleClick" transer placement="right-start">
    ...
    <DropdownMenu slot="list">
      <template v-for="child in children">
        ...
        <DropdownItem v-else-if="children.name" :key="`drop-${child.name}`" :name="child.name"><Icon :size="iconSize" :type="child.icon"/><span class="menu-title">{{ showTitle(child) }}</span></DropdownItem>
      </template>
    </DropdownMenu>
  </Dropdown>
</template>
