<template>
  <Modal
    ...
    title="新增比赛记录"
    style="width: 600px;">
    <Form ref="competition_form" :model="competition" :rules="competition_ruleValidate" style="width: 90%" :label-width="100">
      <FormItem label="比赛名称:" prop="award_name">
        <Input v-model="competition.award_name" placeholder="比赛名称" clearable></Input>
      </FormItem>
      <FormItem label="比赛级别:" prop="level">
        <Input v-model="competition.level" placeholder="比赛级别"  clearable></Input>
      </FormItem>
      <FormItem label="主办单位:" prop="organizer">
        <Input v-model="competition.organizer" placeholder="主办单位"  ></Input>
      </FormItem>
      <FormItem label="学期：" prop="term">
        <Select v-model="competition.term" style="width:200px">
          <Option v-for="item in terms" :value="item.name" :key="item.name">{{ item.name }}</Option>
        </Select>
      </FormItem>
      <FormItem label="比赛时间:" prop="start_time">
        <DatePicker type="datetime" format="yyyy-MM-dd HH:mm:ss" placeholder="获奖时间" v-model="competition.start_time"></DatePicker>
      </FormItem>
    </Form>
...
export default {
  name: "TeacherAddCompetitionModal",
  mixins: [UserMixin],
  props: {
    onOk: Function,
    onCancel: Function,
  ...
  data() {
    return {
      ...
      competition_ruleValidate: {
        ...
        start_time: [{required:true,message:'请选择获奖时间'}],
      }
    }
  ...
  methods: {
    handleOK(){
      this.competition_user.username = this.userInfo.userInfo.username
      this.competition.start_time = dateToString(this.competition.start_time, 'yyyy-MM-dd hh:mm:ss')
      console.log(this.competition)
      console.log(this.competition_user)
      this.$refs.competition_form.validate((valid1) => {
        console.log('valid1', valid1)
        this.valid1 = valid1
      })
      this.$refs.competition_user_form.validate((valid2) => {
  ...
  created() {
    ...
    getCurrentTerms().then((termResp) => {
      this.competition.term = termResp.data.term.name
    })
  },
}
</script>
